...
namespace top
{
    namespace base
    {
        xvcontract_t::xvcontract_t(const std::string & contract_name,xvbstate_t & bind_state)
            :xvexeunit_t((enum_xdata_type)enum_xobject_type_vcontract)
        {
            bind_state.add_ref();
        ...
        //[contract].function
        const xvalue_t   xvcontract_t::execute(const xvmethod_t & op,xvcanvas_t * canvas)//might throw exception for error
        {
            if(is_close())
            {
            ...
            const xvaction_t * action = ((xvaction_t*)op.query_minterface(xvaction_t::enum_obj_type));
            if(action == NULL)
            {
                xerror("xvcontract_t::execute,NOT xvaction_t type for name(%s) of uri(%s)",op.get_method_name().c_str(), op.get_method_uri().c_str());
        ...
        const xvalue_t  xvcontract_TEP0::do_deposit(const xvmethod_t & op,const uint64_t minimal_tgas)
        {
            ...
            ((xvaction_t*)op.query_minterface(xvaction_t::enum_obj_type))->add_used_tgas(minimal_tgas);
        ...
        const xvalue_t  xvcontract_TEP0::do_withdraw(const xvmethod_t & op,const uint64_t minimal_tgas)
        {
        ...
        const xvalue_t  xvcontract_TEP1::do_deposit(const xvmethod_t & op,const uint64_t minimal_tgas)
        {
            if(op.get_method_name() != get_deposit_function_name())
                return xvalue_t(enum_xerror_code_bad_method);
        ...
        const xvalue_t  xvcontract_TEP1::do_withdraw(const xvmethod_t & op,const uint64_t minimal_tgas)
        {
            if(op.get_method_name() != get_withdraw_function_name())
                return xvalue_t(enum_xerror_code_bad_method);
        ...
        const xvalue_t  xvcontract_TEP2::do_transfer(const xvmethod_t & op,const uint64_t minimal_tgas)
        {
            ...
            if(amount_to_transfer < 0)
            {
                xerror("xvcontract_TEP2::do_transfer,negative amount while contract(%s) transfer from(%s) to (%s) by amount(%" PRIu64 ")",get_contract_address().c_str(), op.get_caller_uri().c_str(),receiver_account_value.get_string().c_str(),amount_to_transfer);
        ...
        const xvalue_t  xvcontract_TEP2::do_burn(const xvmethod_t & op,const uint64_t minimal_tgas)
        {
