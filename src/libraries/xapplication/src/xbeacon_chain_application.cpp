    ...
    for (const auto & addr : sys_addr) {
        ...
        for (auto const & property : property_names) {
            ...
            if (nullptr == latest_vblock_state) {
                xerror("xtop_beacon_chain_application::load_last_election_data get target state fail.block=%s", latest_vblock->dump().c_str());
                continue;
            }
            ...
            if (true == latest_vblock_state->string_get(property, result) && !result.empty()) {
                ...
                if ((addr == sys_contract_rec_elect_rec_addr || addr == sys_contract_rec_elect_zec_addr || addr == sys_contract_zec_elect_consensus_addr) && block_height != 0) {
                    ...
                    if (prev_latest_vblock != nullptr) {
                        ...
                        if (nullptr == prev_latest_vblock_state) {
                            xerror("xtop_beacon_chain_application::load_last_election_data get target state fail.block=%s", prev_latest_vblock->dump().c_str());
                            continue;
                        }
                ...
                xerror("xtop_beacon_chain_application::load_last_election_data fail-read property.addr=%s,property=%s", addr.c_str(), property.c_str());
            }
